...
from .model_GMVAE import CondGMVAE
from ..utils import load_Cond, get_proper_device
...
def build_GMVAE(x_channels, cond_channels, hparams, is_training):
    ...
    if hparams.Train.condmodel == "enc":
        graph = CondGMVAE(x_channels, cond_channels, hparams.Gumbel.num_classes, hparams)
    ...
    if graph is not None:
        ...
        if is_training:
            ...
            pre_trained = hparams.Infer.pre_trained_cond
        if pre_trained is not None:
            loaded_step = load_Cond(os.path.basename(pre_trained),
