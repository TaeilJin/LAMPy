...
class FlowNet(nn.Module):
    ...
    def forward(self, z, cond, ee_cond=None, label = None, logdet=0., reverse=False, eps_std=None):
        if not reverse:
            # train x->z
            ...
            z_u_cond = torch.cat((label,cond,masked_init),dim =1)
            for layer in self.layers_u:
            ...
            z_l_cond = torch.cat((label,cond,masked_u),dim =1)
            for layer in self.layers_l:
...
class DAFNetGen(nn.Module):
    ...
    def __init__(self, x_channels, cond_channels, hparams):
        ...
        self.z_shape = [hparams.Train.batch_size // num_device, x_channels, 70]
    ...
    def forward(self, x=None, cond=None, ee_cond=None, label= None, z=None, 
                eps_std=None, reverse=False):
        if not reverse:
            return self.normal_flow(x, cond, ee_cond, label)
        else:
            return self.reverse_flow(z, cond, ee_cond, label,eps_std)

    def normal_flow(self, x, cond, ee_cond, label):
        ...
        # encode
        z, objective = self.flow(x, cond, ee_cond,label=label, logdet=logdet, reverse=False)
    ...
    def reverse_flow(self, z, cond, ee_cond, label,eps_std):
        with torch.no_grad():
            ...
            x = self.flow(z, cond, ee_cond, label = label, eps_std=eps_std, reverse=True)
        return x
