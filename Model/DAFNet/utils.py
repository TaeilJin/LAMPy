...
def _file_at_step(step, b_motion = True):
    if (b_motion == True):
        return "save_{}k{}.pkg".format(int(step // 1000), int(step % 1000))
    else:
        return "save_mcF_{}k{}.pkg".format(int(step // 1000), int(step % 1000))

def _file_best(b_motion = True):
    if (b_motion == True):
        return "trained.pkg"
    else:
        return "trained_mcF.pkg"

def save(global_step, graph, optim, means=None, variance=None,criterion_dict=None, pkg_dir="", is_best=False, max_checkpoints=None, b_motion = True):
    if optim is None:
    ...
    save_path = os.path.join(pkg_dir, _file_at_step(global_step,b_motion))
    best_path = os.path.join(pkg_dir, _file_best(b_motion))
...
def load(step_or_path, graph, optim=None, means=None, variance=None, criterion_dict=None, pkg_dir="", device=None):
    step = step_or_path
    save_path = None
    ...
    if save_path is not None and not os.path.exists(save_path):
        ...
        raise Exception("[Checkpoint]: Failed to find {}".format(save_path))
        #return
    ...
    if state["means"] is not None:
        graph.means= state["means"]
...
def load_Cond(step_or_path, graph, optim=None, criterion_dict=None, pkg_dir="", device=None):
    step = step_or_path
    save_path = None
...
def load_SAMP(step_or_path, graph, optim=None, means=None, variance=None, criterion_dict=None, pkg_dir="", device=None):
    step = step_or_path
    save_path = None
