...
class InferenceNet(nn.Module):
  def __init__(self, x_dim, z_dim, y_dim):
    ...
    hidden_dim = 512
    self.env_dim = 2640
    self.encoder_env = nn.Linear(self.env_dim,hidden_dim)

    # q(y|x)
    self.inference_qyx = torch.nn.ModuleList([
        nn.Linear(x_dim-self.env_dim+hidden_dim, 512),
        nn.ReLU(),
        nn.Linear(512, 512),
        nn.ReLU(),
    ...
    self.inference_qzyx = torch.nn.ModuleList([
        nn.Linear(x_dim-self.env_dim+hidden_dim + y_dim, 512),
        nn.ReLU(),
        nn.Linear(512, 512),
        nn.ReLU(),
        Gaussian(512, z_dim)
  ...
  def forward(self, x, temperature=1.0, hard=0):
    ...
    env = x[:,-self.env_dim:]
    l_env = self.encoder_env(env)

    x = torch.cat((x[:,:-self.env_dim],l_env),dim=-1)
    # q(y|x)
