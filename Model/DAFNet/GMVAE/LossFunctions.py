...
class LossFunctions:
    ...
    distribution = modules.GaussianDiagPrior()
    ...
    def mean_squared_error(self, real, predictions):
      ...
      #return loss.sum(-1).mean()thops.mean(loss,dim=[0,1])
    ...
    def reconstruction_loss(self, real, predicted, rec_type='mse' ):
      ...
      """
      if rec_type == 'mse':
        loss = self.mean_squared_error(real,predicted) #(real - predicted).pow(2)
      elif rec_type == 'bce':
    ...
    def gaussian_loss(self, z, z_mu, z_var, z_mu_prior, z_var_prior):
      ...
      #loss = self.distribution.logp(z_mu,z_var,z) - self.distribution.logp(z_mu_prior,z_var_prior,z)

      #loss = thops.mean(loss,dim=[0,1])
      loss = loss.mean()
      return loss
    ...
    def entropy(self, logits, targets):
      ...
      loss = -torch.mean(torch.sum(targets * log_q, dim=-1))
    
      #loss = thops.sum(targets * log_q, dim=[1])
      #loss = -thops.mean(loss,dim=[0])
