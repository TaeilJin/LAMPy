...
class GaussianDiagPrior:
    ...
    def likelihood(mean, var, x):
        ...
        lnL = -1/2 * { ln|Var| + ((X - Mu)^T)(Var^-1)(X - Mu) + kln(2*PI) }
              ...
              Var = logs ** 2
        """
        return -0.5 * (GaussianDiagPrior.Log2PI + torch.log(var) + torch.pow(x - mean, 2) / var)

    @staticmethod
    def logp(mean, logs, x):
        likelihood = GaussianDiagPrior.likelihood(mean, logs, x)
        return thops.sum(likelihood, dim=[1, 2])

    @staticmethod
    def sample(mean, var, eps_std=None,device=None):
        eps_std = eps_std or 1
        eps = torch.normal(mean=torch.zeros_like(mean),
                           std=torch.ones_like(var) * eps_std)
        return (mean + var * eps).to(device)
...
class ScaleDotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None, e=1e-12):
        ...
        # 2. apply masking (opt)
        if mask is not None:
            #score = score.masked_fill(mask <1e-4, -1e-9)
            score = score *mask
...
class MAB(nn.Module):
    ...
    def gen_mask(self, input):
        nBatch, nFeats, nDim = input.shape # (BxT,C)
        
        mask = torch.ones_like(input)
        mask[(torch.abs(input) < 1e-4)] = -1e-12
        
        mask = mask[:,:,0].repeat_interleave(nFeats,dim=0).unsqueeze(1).reshape(nBatch,nFeats,nFeats)
        
        #mask = mask.repeat_interleave(self.num_heads,dim=0).unsqueeze(1).reshape(nBatch,-1,nFeats,nFeats)
    ...
    def forward(self, Q, K):
        ...
        # 3. scale dot product to compute similarlity
        ...
        # input 
        # [batch*Timestep,nFeats,1]
        # [batch*Timestep,nFeats,D]
        ...
        # # [nHead?,batch*Timestep,nFeats,D//2]

        # W_mat = Q_.bmm(K_.transpose(2,3))/math.sqrt(self.dim_V)
        # # condition masking
        # W_mat = mask * W_mat # (nHead, batch*Timesteps,c,c)

        # # attentioned input value
        # A = torch.softmax(W_mat, -1)
