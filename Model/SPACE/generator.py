...
class GenerateFunc():
    ...
    def generate_sample_withRef(self,graph, joints_all, vel_all, len_all, surface_all,\
        ...
        ref_scaled_all = self.inv_standardize(np.concatenate((autoreg_all,vel_all), axis=-1),scaler)
        ref_joints_all_unscaled = ref_scaled_all[:,10:,:75]
        vel_all_unscaled = ref_scaled_all[:,10:,-3:]

        return sampled_all[:,10:,:], ref_joints_all_unscaled,vel_all_unscaled